...
class City(BaseModel):
    ...
    MunicipalSalesTax: (
        float | None
    )  # Municipal sales tax as fraction, e.g. 0.06 means 6%
    ...
    avg_elec_usage: List[float] = [
        0.0
    ] * 12  # 12-item array of average residential kWh usage for each month
...
class Utility(BaseModel):
    ...
    ]  # Array of (kWh block limit, $/kWh for block) for all rate blocks
...
class Fuel(BaseModel):
    ...
    price_col: (
        str | None
    )  # Name of column in City model that contains the price per unit of this fuel in that City
    dhw_effic: (
        float  # Average efficiency of a domestic hot water system burning that fuel
    )
    effic_choices: List[
        Tuple[str, float]
    ]  # Array of (space heating system type, efficiency) for this fuel type
...
class TMYdataset(BaseModel):
    ...
    hourly_data: Dict[str, List[float | int]] | None = (
        None  # dictionary of hourly data, keys are column names
    )
